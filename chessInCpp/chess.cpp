...
bool checkIfCheck(std::string board, char player) {
    ...
    for(int i = 0; i < board.length(); i++) {
        ...
        if(piece[0] == player && piece[1] == 'K') {
            ...
            break;}}
    if(kingCell.empty()) return false;
    for(int i = 0; i < board.length(); i++) {
        std::string piece = getPieceName(board[i]);
        if(piece[0] == 'n' || piece[0] == player || piece[1] == 'K') {continue;}
        std::string moves = getOrDoMoves(getCellCode(i), "N1", false, true, board);
        if(moves.find(kingCell) != std::string::npos) {return true;}}
    return false;}
...
int boardValCalc(std::string board,char player,int count){
  int tVal=0;
  char otherPlayer;if(player=='w'){otherPlayer='b';}else{otherPlayer='w';}
  for(int i=0;i<board.length();i++){
  ...
  for(int i=0;i<piecesToCheck.length();i+=2){
    ...
    if(moves.length()!=0){
      ...
      for(int j=0;j<sortedMoves.length();j+=2){
        ...
        if(checkPieceAmm((otherPlayer == 'w') ? 'k' : 'K', nBoard) == 0) {return cell + fCell + std::to_string(100000);}
        int score=0;
        if(depth>0){
...
int main(){
  ...
  while(true){
    ...
    char player='w';std::string word="White";
    ...
    if(checkPieceAmm('K',board)==0){std::cout<<"Black wins!";break;}
    if(checkPieceAmm('k',board)==0){std::cout<<"White wins!";break;}
  }
  return 0;
}
